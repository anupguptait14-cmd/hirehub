...
import { Mail, Lock, User, UserPlus, Briefcase, UserCheck, Building2 } from 'lucide-react';

export const Register = () => {
  const [searchParams] = useSearchParams();
  const roleParam = searchParams.get('role');
  const initialRole = roleParam === 'recruiter' ? 'recruiter' : 'candidate';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.role === 'recruiter') navigate('/recruiter/dashboard');
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-gray-500">Join thousands of job seekers and hiring companies</p>
        </div>

        {/* Public Role Selector Tabs (Candidate & Employer only) */}
